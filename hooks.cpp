...
DECLSPEC_NOINLINE bool __fastcall SetupBonesLocal(void* ECX, void* EDX, matrix3x4_t* pBoneToWorldOut, int nMaxBones, int boneMask, float currentTime) {
	Player* player = (Player*)(uintptr_t(ECX) - 4);

	if (player->index() > 64 || player->index() < 0 || player != g_cl.m_local)
...
void Hooks::init() {
	...
	m_SetupBones = (DWORD)DetourFunction((byte*)setupBones, (byte*)SetupBonesLocal);
